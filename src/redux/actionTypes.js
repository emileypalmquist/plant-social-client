// Plants reducer action types
export const ADD_PLANTS = "ADD_PLANTS";
export const ADD_USER_PLANTS = "ADD_USER_PLANTS";
export const SET_USER_PLANT_SHOW = "SET_USER_PLANT_SHOW";
export const ADD_CARE_NOTE = "ADD_CARE_NOTE";
export const REMOVE_CARE_NOTE = "REMOVE_CARE_NOTE";
export const ADD_COMMENT = "ADD_COMMENT";
export const REMOVE_COMMENT = "REMOVE_COMMENT";
export const ADD_USER_PLANT_LIKE = "ADD_USER_PLANT_LIKE";
export const REMOVE_USER_PLANT_LIKE = "REMOVE_USER_PLANT_LIKE";
export const ADD_CARE_NOTE_LIKE = "ADD_CARE_NOTE_LIKE";
export const REMOVE_CARE_NOTE_LIKE = "REMOVE_CARE_NOTE_LIKE";
export const ADD_COMMENT_LIKE = "ADD_COMMENT_LIKE";
export const REMOVE_COMMENT_LIKE = "REMOVE_COMMENT_LIKE";
export const SET_LIKED_USER_PLANTS = "SET_LIKED_USER_PLANTS";
export const ADD_PLANT = "ADD_PLANT";
export const ADD_FAVORITE_PLANT = "ADD_FAVORITE_PLANT";
export const REMOVE_FAVORITE_PLANT = "REMOVE_FAVORITE_PLANT";

//*****************************//
// User reducer action types
export const SET_USER = "SET_USER";
export const SIGN_OUT = "SIGN_OUT";
export const ADD_USER_PLANT = "ADD_USER_PLANT";
export const REMOVE_USER_PLANT = "REMOVE_USER_PLANT";
export const UPDATE_USER_PLANT = "UPDATE_USER_PLANT";

//*****************************//
// status reducer action types
export const ADD_ERROR = "ADD_ERROR";
export const REMOVE_ERROR = "REMOVE_ERROR";
export const SET_LOADING = " SET_LOADING";

//*****************************//
// greenhouse reducer action types
export const SET_GREENHOUSE = "SET_GREENHOUSE";
export const RESET_GREENHOUSE = "RESET_GREENHOUSE";
